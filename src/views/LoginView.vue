...
async function submit() {
  ...
  if (!name.value.trim() || !/^\d{4}$/.test(pin.value)) {
    toast.error('이름과 PIN(4자리 숫자)을 입력하세요');
    return;
  }
  ...
  try {
    ...
    if (axios.isAxiosError(err)) {
      ...
      else if (status === 401) toast.error('이름 또는 PIN이 일치하지 않습니다');
  ...
  <div class="flex items-center justify-center h-screen bg-gray-100">
    ...
    >
      ...
      <p class="text-xs text-gray-500 text-center -mt-2">
        처음 입장이면 사용할 PIN을 새로 설정하고, 재입장이면 같은 PIN을 입력하세요.
      </p>
      ...
      <label class="flex flex-col gap-1 text-sm">
        <span class="text-gray-700">PIN (숫자 4자리)</span>
        <input
          v-model="pin"
          type="password"
          inputmode="numeric"
